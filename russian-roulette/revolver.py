...
async def spin(session: CommandSession):
    if session.ctx['message_type'] == 'private':
        ...
        if group not in data:
            ...
            data[group]['curnum'] = 0
            data[group]['next'] = 1
            data[group]['bantime'] = 0
            
            

        if data[group]['curnum'] <= 0:
            msg = '欢迎参与紧张刺激的俄罗斯轮盘，请输入要填入的子弹数目(最多6颗)'
            bullet = int(session.get('bullet', prompt=msg))
            if bullet == 6:
                ans = session.get('ans', prompt="你认真的？(y/n)")
                if ans == 'y':
                    msg = '请输入失败者禁言的时长（以秒为单位）'
                    bantime = int(session.get('bantime', prompt=msg))
                    if bantime > 2592000:
                        session.finish("禁言时间不能大于30天！")
                    elif bantime < 0:
                        session.finish("禁言时间应为非负整数！")
                    else:
                        data[group]['curnum'] = bullet
                        data[group]['next'] = 0
                        data[group]['bantime'] = bantime
                    await session.send("设定完毕，共装填了%s发子弹,设定时长为%ss，发送“开枪”参与游戏"%(bullet,bantime))
                else:
            ...
            if data[group]['next'] == 0:
                await session.send("砰！你倒在了血泊中......")
                await bot.set_group_ban(group_id=group, user_id=int(user), duration=data[group]['bantime'])
                ...
                if data[group]['curnum'] <= 0:
                    ...
                    await session.send("有请下一位.还剩%d发子弹" % data[group]['curnum'])
            else:
                data[group]['next'] -= 1
                msg = "这一枪没有子弹，下一位，还剩%d发" % data[group]['curnum']
